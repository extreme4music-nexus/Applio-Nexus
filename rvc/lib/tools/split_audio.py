    ...
    if os.path.exists(config_path):
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                return (
                    data.get("parallel", False),
                    data.get("lock_pitch", False),
                    data.get("num_workers", None),
                )
        except Exception:
            pass
...
def parallel_inference_mapping(
    ...
    **kwargs,
    ...
    ui_parallel_enabled = (
        os.environ.get("APPLIO_PARALLEL", "false").lower() == "true"
    ) or saved_parallel
    ui_lock_pitch_enabled = (
        os.environ.get("APPLIO_PARALLEL_LOCK_PITCH", "false").lower() == "true"
    ) or saved_lock_pitch
    ...
    if not ui_parallel_enabled or not split_audio_enabled:
        print(
            "[Parallel Engine] Parallel optimization disabled or Split Audio unchecked. Running default loop."
        )
        converted_chunks = []
        try:
    ...
    print(
        f"[Parallel Engine] Orchestrator online. Total chunks to process: {len(chunks)}"
    )
    ...
    if ui_lock_pitch_enabled and proposed_pitch_active and len(chunks) > 0:
        print(
            "[Parallel Engine] Force Uniform Pitch Active. Scanning for first active vocal chunk to establish master baseline offset..."
        )
        ...
        try:
            ...
            if 0 < f0_cohesive < proposed_pitch_threshold:
                ...
                calculated_offset = int(
                    np.round(12 * np.log2(proposed_pitch_threshold / f0_cohesive))
                )
                pitch = pitch + calculated_offset
                print(
                    f"[Parallel Engine] First chunk tracking complete. Applied uniform transposition shift: {calculated_offset} semitones."
                )
            else:
                print(
                    "[Parallel Engine] First voice chunk range is already optimal. Maintaining default baseline keys."
                )
            ...
            print(
                f"[Parallel Engine] Warning: Native chunk pitch tracking encountered an error ({pyin_err}). Falling back to chunk-level tracking."
            )
    ...
    is_gpu_mode = "cpu" not in saved_device and (
        torch.cuda.is_available() or "cuda" in saved_device
    )
    ...
    if is_gpu_mode:
        try:
            ...
            print(
                f"[Parallel Engine] Target backend: GPU. Available VRAM: {vram_free_gb:.2f} GB. Max safe concurrent workers: {max_workers}"
            )
        except Exception:
            max_workers = min(max_workers, 2)
            print(
                f"[Parallel Engine] Hardware device query failed. Throttling concurrent worker capacity safely to: {max_workers}"
            )
    else:
        ...
        print(
            f"[Parallel Engine] Target backend: CPU. Available RAM: {available_sys_ram_gb:.2f} GB. Max safe concurrent workers: {max_workers}"
        )
    ...
    chunk_groups = [
        chunks[x : x + max_workers] for x in range(0, total_chunks, max_workers)
    ]
    print(
        f"[Parallel Engine] Divided work into {len(chunk_groups)} execution group blocks to prevent out-of-memory issues."
    )
    ...
    try:
        ...
        for group_idx, current_group in enumerate(chunk_groups):
            print(
                f"[Parallel Engine] Processing Execution Group [{group_idx + 1}/{len(chunk_groups)}] (Size: {len(current_group)} chunks)..."
            )

            with concurrent.futures.ThreadPoolExecutor(
                max_workers=len(current_group)
            ) as executor:
                futures = []
        ...
        print(
            "[Parallel Engine] All execution batches completed successfully. Merging chunks."
        )
        return final_results

    except Exception as e:
        print(
            f"[Parallel Engine] Critical failure inside asynchronous group worker processing: {e}"
        )
        raise e
    finally:
        if "futures" in locals():
            del futures
        if "converted_chunks_map" in locals():
            del converted_chunks_map
        import gc
