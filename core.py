...
# Add this line to the top of core.py with the other imports:
from rvc.train.process.dataset_scrub import scrub_audio
...
def run_infer_script(
    ...
    delay_mix: float = 0.5,
    sid: int = 0,
    **kwargs, # <--- ADD THIS to catch the incoming dictionary
):
    kwargs_main = {
    ...
    # Merge the Nexus performance settings into the pipeline dictionary
    kwargs_main.update(kwargs)
    
    infer_pipeline = import_voice_converter()
    infer_pipeline.convert_audio(**kwargs_main)
...
# TTS
# TTS (Updated to accept Nexus Engine parameters)
def run_tts_script(
    tts_file: str,
    tts_text: str,
    tts_voice: str,
    ...
    **kwargs,  # <--- ADD THIS to catch the incoming dictionary
    ...
    # ... (Keep existing subprocess command_tts logic) ...
    command_tts = [
        *map(
            str,
            [
    ...
    # Update the conversion call to unpack the Nexus parameters
    infer_pipeline.convert_audio(
        ...
        embedder_model_custom=embedder_model_custom,
        sid=sid,
        # Unpack the Nexus parameters stored in kwargs here:
        **kwargs
    )
...
#Dataset Scrub
def run_dataset_scrubber(input_folder, output_folder, use_gate, gate_db, use_eq, low_cut, high_cut, use_limiter, limiter_db, gain_mode, manual_gain_db):
    if not input_folder or not os.path.exists(input_folder):
        return "Error: Input folder target path does not exist."
    results = []
    for filename in os.listdir(input_folder):
        if filename.endswith((".wav", ".mp3", ".flac")):
            path = os.path.join(input_folder, filename)
            success, msg = scrub_audio(
                path, output_folder, 
                use_gate, gate_db, 
