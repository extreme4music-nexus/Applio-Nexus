...
def inference_tab():
    ...
    with gr.Tab(i18n("Single")):
        ...
        def enforce_terms(terms_accepted, *args):
            ...
            try:
                # Extract the 12 Blending/Matrix parameters from the tail of the stack
                embedder_model_custom_b       = args_list.pop()
                embedder_model_b              = args_list.pop()
                blend_bias                    = float(args_list.pop())
                blend_velocity_switching      = args_list.pop()
                blend_crossover_freq          = float(args_list.pop())
                performance_vibrato_intensity = float(args_list.pop())
                performance_vibrato_style     = args_list.pop()
                performance_breathiness       = float(args_list.pop())
                performance_grit              = float(args_list.pop())
                f0_method_b                   = args_list.pop()
                index_file_b                  = args_list.pop()
                model_file_b                  = args_list.pop()

                # After popping the 12 trailing elements, args_list contains exactly the 59 base arguments
                original_output_path = args_list[6]
                ...
                # Task Allocation Group A (Primary Model Configuration)
                args_worker_a = list(args_list)
                args_worker_a[6] = path_worker_a

                # Task Allocation Group B (Secondary Model/Nexus Configuration)
                args_worker_b = list(args_list)
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                    ...
                    if future_b:
                        ...
                        actual_path_b = res_b[2] if isinstance(res_b, tuple) else path_worker_b
                    ...
                    if os.path.exists(actual_path_a):
                        if os.path.exists(original_output_path):
                            os.remove(original_output_path)
                        shutil.move(actual_path_a, original_output_path)
    ...
    convert_button1.click(
        ...
        inputs=[
            ...
            blend_crossover_freq,
            blend_velocity_switching,
            blend_bias,
            embedder_model_b,
